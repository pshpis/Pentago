...
$logger = new Logger('Websocket Logger');
$logger->pushHandler(new StreamHandler($ROOT_PATH.'/logs/websocket.log', Logger::DEBUG));
$logger->pushHandler(new FirePHPHandler());

$logger -> debug('Вебсокет сервер запущен');


$worker = new Workerman\Worker("websocket://0.0.0.0:49021/controller/websocket_controller.php");
$worker -> count = 1;

$users_connections = array();
$users_id = array();

$worker -> onMessage = function ($connection, $data) use (&$game, &$current_user, &$users_connections, &$worker, &$users_id){
    global $logger;
    ...
    if (isset($data['make_game']) && $data['make_game']){
        $logger -> info('Пользователь '.$user_id.' начал поиск игры');
        $users_id[] = $user_id;
        $users_connections[$user_id] = $connection;
        $users_connections[$user_id] -> send(json_encode("Server get your message. It was ".json_encode($data)."  You also can see users ".json_encode($users_id)));
    ...
    if (isset($data['end_search']) && $data['end_search']){
        $logger -> info('Пользователь '.$user_id.' не дождался пока мы ему найдем опонента');
        $game -> delete_session();
        return;
    }

    if (isset($data['opponent_leave']) && $data['opponent_leave']){
        opponent_leave($game, $user_id, $connection);
        return;
    }

    if (isset($data['give_up']) && $data['give_up']){
        if ($game -> is_game_session_finish()) $logger -> info('Пользователь '.$user_id.' сдался');
        give_up($user_id, $connection, $users_connections, $game);
        return;
    }
    ...
    if ($rotate_quarter !== -1 && $rotate_dir !== -2){
        ...
        if ($game -> make_rotate($rotate_quarter, $rotate_dir, $user_id)){
            ...
            $logger -> debug("Пользователь $user_id сделал поворот четверти $rotate_quarter в направении $rotate_dir. Информация об игре - ".$game -> get_info());
...
$worker -> onConnect = function ($connection){
    global $logger;
    $logger -> debug('К вебсокет серверу подключился новый пользователь');
};

$worker -> onClose = function ($connection){
    global $logger;
    $logger -> debug('Пользователь отключился от вебсокет сервера');
};


Workerman\Worker::runAll();

function make_game($user_id, $connection, $data, $users_connections){
    $users_id[] = $user_id;
    $users_connections[$user_id] = $connection;
    $users_connections[$user_id] -> send(json_encode("Server get your message. It was ".json_encode($data)."  You also can see users ".json_encode($users_id)));
    $start_time = $data['start_time'];
...
function opponent_leave($game, $user_id, $connection){
...
function give_up($user_id, $connection, $users_connections, $game){
    $opponent_connection = $users_connections[$game->get_opponent_id($user_id)];
    $game -> give_up($user_id);
