...
const ws = new WebSocket('ws://localhost:49021/controller/websocket_controller.php');
...
ws.send = function(x){
    ws.send1(x);
    console.log('Websocket send data is ' + x);
};

ws.onopen = async function (event) {
    await set_id();
    console.log('My id set succes');
...
ws.onmessage = function (event) {
    console.log("Data from server is " + event.data);
...
ws.onclose = function(event) {
    console.log("CONNNNNNNECTION WAS CLOOOOSED!!!!!!!!!!!!");
    if (event.wasClean) {
