...
class Pentago_Session extends Online_Game_Session{
    protected string $cond; // conditional of the desk 36 symbol
    protected string $rotate; // string of 4 symbols. watching rotate of every quarter

    protected int $step_complete; // step is moving figure
    protected int $rotate_complete; // rotate is changing rotation quarter
    ...
    function __construct(int $user_id){
        ...
        $logger = get_default_log_chanel();
        $logger -> debug('Объект класса Pentago_Session был создан пользователем '.$user_id.'. Его id - '.$this->get_id());
        $mysql = openDB();
    ...
    public function make_step($pos, $user_id): bool{
        ...
        if ($this->step_complete % 2 + 1 === $side){
            $logger = get_default_log_chanel();
            $logger -> debug('Пользователь '.$user_id.' делает шаг '.$pos);
            $this->cond[$pos] = $side;
    ...
    public function make_rotate(int $quarter, int $dir, int $user_id): bool{
        ...
        $logger = get_default_log_chanel();
        $logger -> debug('Пользователь '.$user_id.' делает поворот четверти '.$quarter.' в сторону '.$dir);
